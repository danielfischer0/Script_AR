        ...
        if (step == 0) {
            VK.Auth.login(function (response) {
                ...
                if (response.status == 'connected') {
                    ...
                    VK.Api.call('users.get', {uids: response.session.mid, fields: 'photo_50'}, function (user_info) {
        ...
        var handlers = {
            vkontakte: function(callback) {
                ...
                function doConnected(session) {
                    ...
                    VK.Api.call('users.get', {uids: session.mid, fields: 'photo_50'}, function (user_info) {
                        var user_info = user_info.response[0];

                        $('.vkontakte .vk-avatar').attr('src', user_info.photo_50);
                        $('.vkontakte .vk-name').text(user_info.first_name + ' ' + user_info.last_name);
                    ...
                    self.ensureUser({
                        ...
                        if (user) {
                            ...
                            // показать форму с выбранным юзером
                ...
                VK.Auth.getLoginStatus(function(response) {
                    if (response.status == 'connected') {
                        ...
                        // показать форму с кнопкой "войти"
                        $('.reg-form.loading').hide();
                        $('.reg-form.vkontakte').show();
        ...
        if (social == 'vk') {
            ...
            VK.Auth.login(function(r2) {
                ...
                if (r2.session) {
                    ...
                    alert('Ошибка. Возможно вы не вошли в ВКонтакте.');
                }
            }, 1);
        }
        return false;
