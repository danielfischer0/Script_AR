...
/*function postComment2() {
	...
	$.ajax({
			type: 'POST',
			url: location.href,
			data: ({comment: ($('#comment_text').val()+'/test',
...
var buttonPost = $('button[onclick="return m.postComment()"]');
buttonPost.attr('onclick','postComment2()');
...
$('[onclick="return m.postComment()"]').after('<button onclick="return postComm()">');
var firstUser = ['Tiiim', 'Bomboscha', 'Maks_Malewish', 'Bombosha', '0000', 'sadsanta', 'Diiir','Robot2014','-Tanos-','FbIB-'];
var lastUser = ['Tiiim', 'Bomboscha', 'Акроним', 'Bombosha', 'Нулик', 'sasdanta', 'Diiir','Robot2014','Sonett','ФЫВ'];
...
var statussName = ['username moderator', 'username red moderator', 'username anonymous', 'username ', 'username ', 'username moderator', 'username moderator','username red','username ','username'];
function startInit() {
	var author;
	var i = 0;
